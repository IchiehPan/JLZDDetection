...
//NET_DVR_Login()参数结构
    ...
    public static class NET_DVR_FACE_DETECTION extends Structure {// 人脸侦测报警信息
        public int dwSize; // 结构大小
        public int dwRelativeTime; // 相对时标
        public int dwAbsTime; // 绝对时标
        public int dwBackgroundPicLen; // 背景图的长度，为0表示没有图片，大�?0表示有图�?
        public NET_VCA_DEV_INFO struDevInfo; // 前端设备信息
        public NET_VCA_RECT[] struFacePic = new NET_VCA_RECT[MAX_FACE_PIC_NUM/* 30 */];// 人脸子图区域
        public byte[] byFacePicNum;// 子图数量
        public byte[] byUploadEventDataType;// 图片数据长传方式�?0-二进制数据，1-URL
        public int wDevInfoIvmsChannelEx; // 与NET_VCA_DEV_INFO里的byIvmsChannel含义相同，能表示更大的�?��?��?�客户端用byIvmsChannel能继续兼容，但是�?大到255。新客户端版本请使用wDevInfoIvmsChannelEx�?
        public byte[] byRes = new byte[252];
        public Pointer pBackgroundPicpBuffer; // 背景图的图片数据
    }
    ...
    public static class NET_DVR_AID_INFO extends Structure {
        ...
        public byte[] byRes2 = new byte[37];  // 保留字节
    }
    ...
    public static class NET_VCA_FACESNAP_RESULT extends Structure {
        ...
        public NET_VCA_DEV_INFO struDevInfo; // 前端设备信息
        public int dwFacePicLen; // 人脸子图的长度，�?0表示没有图片，大�?0表示有图�?
        public int dwBackgroundPicLen; // 背景图的长度，为0表示没有图片，大�?0表示有图�?(保留)
        public byte bySmart; // IDS设备返回0(默认�?)，Smart Functiom Return 1
        public byte byAlarmEndMark;// 报警结束标记0-保留�?1-结束标记（该字段结合人脸ID字段使用，表示该ID对应的下报警结束，主要提供给NVR使用，用于判断报警结束，提取识别图片数据中，清晰度最高的图片�?
        public byte byRepeatTimes; // 重复报警次数�?0-无意�?
        public byte byUploadEventDataType;// 人脸图片数据长传方式�?0-二进制数据，1-URL
        public NET_VCA_HUMAN_FEATURE struFeature; // 人体属�??
        public float fStayDuration; // 停留画面中时�?(单位: �?)
        public byte[] sStorageIP = new byte[16]; // 存储服务IP地址
        public short wStoragePort; // 存储服务端口�?
        public short wDevInfoIvmsChannelEx; // 与NET_VCA_DEV_INFO里的byIvmsChannel含义相同，能表示更大的�?��?��?�客户端用byIvmsChannel能继续兼容，但是�?大到255。新客户端版本请使用wDevInfoIvmsChannelEx�?
        public byte[] byRes1 = new byte[16]; // 保留字节
        public Pointer pBuffer1; // 人脸子图的图片数�?
...
//    }
    ...
    public static class NET_VCA_BLACKLIST_INFO extends Structure {
        ...
        public Pointer pFCAdditionInfoBuffer;//抓拍库附加信息数据指针
        public byte[] byRes2 = new byte[4];
    }
